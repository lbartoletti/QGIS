...
void QgsBezierData::calculateSymmetricHandles( QVector<QgsPoint> &controlPoints, int anchorIndex, const QgsPoint &mousePosition )
{
  ...
  QgsPoint *handleFollow = ( anchorIndex + 1 < controlPoints.size() ) ? &controlPoints[anchorIndex + 1] : nullptr;
  QgsPoint *handleOpposite = ( anchorIndex - 1 >= 0 ) ? &controlPoints[anchorIndex - 1] : nullptr;
