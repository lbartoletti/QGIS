...
void QgsSnappingLayerDelegate::setEditorData( QWidget *editor, const QModelIndex &index ) const
{
  ...
  if ( index.column() == QgsSnappingLayerTreeModel::TypeColumn )
  {
    ...
    if ( tb )
    {
      ...
      actions.at( 0 )->setChecked( type & QgsSnappingConfig::SnappingTypes::VertexFlag );
      actions.at( 1 )->setChecked( type & QgsSnappingConfig::SnappingTypes::SegmentFlag );
      actions.at( 2 )->setChecked( type & QgsSnappingConfig::SnappingTypes::AreaFlag );
      actions.at( 3 )->setChecked( type & QgsSnappingConfig::SnappingTypes::CentroidFlag );
      actions.at( 4 )->setChecked( type & QgsSnappingConfig::SnappingTypes::MiddleOfSegmentFlag );
    }
  }
...
void QgsSnappingLayerDelegate::setModelData( QWidget *editor, QAbstractItemModel *model, const QModelIndex &index ) const
{
  if ( index.column() == QgsSnappingLayerTreeModel::TypeColumn )
  {
    ...
    if ( t )
    {
      ...
      QgsSnappingConfig::SnappingTypeFlag type = QgsSnappingConfig::SnappingTypes::NoSnapFlag;
      if ( actions.at( 0 )->isChecked() )
        type = static_cast<QgsSnappingConfig::SnappingTypeFlag>( type | QgsSnappingConfig::SnappingTypes::VertexFlag );
      if ( actions.at( 1 )->isChecked() )
        type = static_cast<QgsSnappingConfig::SnappingTypeFlag>( type | QgsSnappingConfig::SnappingTypes::SegmentFlag );
      if ( actions.at( 2 )->isChecked() )
        type = static_cast<QgsSnappingConfig::SnappingTypeFlag>( type | QgsSnappingConfig::SnappingTypes::AreaFlag );
      if ( actions.at( 3 )->isChecked() )
        type = static_cast<QgsSnappingConfig::SnappingTypeFlag>( type | QgsSnappingConfig::SnappingTypes::CentroidFlag );
      if ( actions.at( 4 )->isChecked() )
        type = static_cast<QgsSnappingConfig::SnappingTypeFlag>( type | QgsSnappingConfig::SnappingTypes::MiddleOfSegmentFlag );
...
QVariant QgsSnappingLayerTreeModel::data( const QModelIndex &idx, int role ) const
{
  ...
  else
  {
    ...
    if ( idx.column() == TypeColumn )
    {
      if ( role == Qt::DisplayRole )
      {
        return QgsSnappingConfig::snappingTypeFlagToString( ls.typeFlag(), 2 );
      }
