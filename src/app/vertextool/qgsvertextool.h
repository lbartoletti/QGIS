...
class APP_EXPORT QgsVertexTool : public QgsMapToolAdvancedDigitizing
{
  ...
  private:
     ...
     * \param movingCtrlPointIndices The indices of the control points being moved.
    ...
    */
     ...
     * \param layer The vector layer (nullptr if already in map coordinates)
     * \param controlPoints The control points to transform
     * \returns Control points in map coordinates
     * \since QGIS 4.0
     */
    QgsPointSequence transformNurbsControlPointsToMap( QgsVectorLayer *layer, const QgsPointSequence &controlPoints );
  ...
  private:
    ...
    struct NurbsBand
    {
        ...
        bool isPolyBezier = false;            //!< Whether it's a Poly-Bézier curve \since QGIS 4.0
         ...
         * \param customKnots Optional custom knot vector (if empty, uses band's stored knots)
         * \param customWeights Optional custom weight vector (if empty, uses band's stored weights)
         */
