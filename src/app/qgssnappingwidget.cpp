...
void QgsSnappingWidget::projectSnapSettingsChanged()
{
  ...
  if ( config.typeFlag() & QgsSnappingConfig::SnappingTypes::VertexFlag )
  {
    mTypeButton->setDefaultAction( mVertexAction );
    mVertexAction->setChecked( true );
  }
  if ( config.typeFlag() & QgsSnappingConfig::SnappingTypes::SegmentFlag )
  {
    mTypeButton->setDefaultAction( mSegmentAction );
    mSegmentAction->setChecked( true );
  }
  if ( config.typeFlag() & QgsSnappingConfig::SnappingTypes::AreaFlag )
  {
    mTypeButton->setDefaultAction( mAreaAction );
    mAreaAction->setChecked( true );
  }
  if ( config.typeFlag() & QgsSnappingConfig::SnappingTypes::CentroidFlag )
  {
    mTypeButton->setDefaultAction( mCentroidAction );
    mCentroidAction->setChecked( true );
  }
  if ( config.typeFlag() & QgsSnappingConfig::SnappingTypes::MiddleOfSegmentFlag )
  {
...
void QgsSnappingWidget::typeButtonTriggered( QAction *action )
{
  ...
  {
    type ^= static_cast<int>( QgsSnappingConfig::SnappingTypes::VertexFlag );
  }
  else if ( action == mSegmentAction )
  {
    type ^= static_cast<int>( QgsSnappingConfig::SnappingTypes::SegmentFlag );
  }
  else if ( action == mAreaAction )
  {
    type ^= static_cast<int>( QgsSnappingConfig::SnappingTypes::AreaFlag );
  }
  else if ( action == mCentroidAction )
  {
    type ^= static_cast<int>( QgsSnappingConfig::SnappingTypes::CentroidFlag );
  }
  else if ( action == mMiddleAction )
  {
    type ^= static_cast<int>( QgsSnappingConfig::SnappingTypes::MiddleOfSegmentFlag );
  }
