...
QgsNurbsCurve::QgsNurbsCurve( const QVector<QgsPoint> &ctrlPoints, int degree,
...
bool QgsNurbsCurve::isBezier() const
{
  ...
  return ( !isRational() && mDegree > 0 &&
           mControlPoints.size() == mDegree + 1 );
}
...
bool QgsNurbsCurve::isClosed() const
{
  ...
  bool closed = qgsDoubleNear( startPt.x(), endPt.x() ) &&
                qgsDoubleNear( startPt.y(), endPt.y() );
...
bool QgsNurbsCurve::isClosed2D() const
{
  ...
  return qgsDoubleNear( startPt.x(), endPt.x() ) &&
         qgsDoubleNear( startPt.y(), endPt.y() );
}

QgsLineString *QgsNurbsCurve::curveToLine( double tolerance,
    SegmentationToleranceType toleranceType ) const
{
...
bool QgsNurbsCurve::equals( const QgsCurve &other ) const
{
  ...
  if ( o->mDegree != mDegree || o->mClosed != mClosed ||
       o->mControlPoints.size() != mControlPoints.size() )
  {
    return false;
  }

  if ( o->mWeights.size() != mWeights.size() ||
       o->mKnots.size() != mKnots.size() )
  {
    return false;
  }
...
void QgsNurbsCurve::scroll( int firstVertexIndex )
{
  ...
  std::rotate( mControlPoints.begin(),
               mControlPoints.begin() + firstVertexIndex,
               mControlPoints.end() );
  std::rotate( mWeights.begin(),
               mWeights.begin() + firstVertexIndex,
               mWeights.end() );
  ...
  if ( !mKnots.isEmpty() && firstVertexIndex < mKnots.size() )
  {
    ...
    std::rotate( mKnots.begin(),
                 mKnots.begin() + firstVertexIndex,
                 mKnots.end() );
...
bool QgsNurbsCurve::fromWkb( QgsConstWkbPtr &wkb )
{
  ...
  {
    // Read byte order for this point (1 byte) - SFCGAL compatibility
    char pointEndianness;
    wkb >> pointEndianness;
    // Note: In QGIS WKB implementation, we use the global endianness from header
    // The per-point endianness is read for SFCGAL compatibility but not applied
...
bool QgsNurbsCurve::fromWkt( const QString &wkt )
{
  ...
  // NURBSCURVE format examples (nouveau format compact):
  // NURBSCURVE(2, (0 0, 5 10, 10 0))
  // NURBSCURVE(2, (0 0, 5 10, 10 0), (1, 2, 1))
  // NURBSCURVE(2, (0 0, 3 6, 6 3, 9 0), (1, 1, 1, 1), (0, 0, 0, 0.5, 1, 1, 1))
...
bool QgsNurbsCurve::fuzzyEqual( const QgsAbstractGeometry &other, double epsilon ) const
{
  ...
  if ( mDegree != o->mDegree || mClosed != o->mClosed ||
       mControlPoints.size() != o->mControlPoints.size() ||
       mWeights.size() != o->mWeights.size() ||
       mKnots.size() != o->mKnots.size() )
  {
    return false;
  }
...
QString QgsNurbsCurve::asWkt( int precision ) const
{
  ...
  else
  {
    ...
    // Add degree first (nouveau format compact)
    ...
    // Follow principle: "Liberal in what you accept, conservative in what you send"
    if ( !mWeights.isEmpty() )
    {
      wkt += QLatin1String( ", (" );
      for ( int i = 0; i < mWeights.size(); ++i )
