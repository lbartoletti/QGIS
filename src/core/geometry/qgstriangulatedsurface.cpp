...
json QgsTriangulatedSurface::asJsonObject( int ) const
{
  QgsDebugError( QStringLiteral( "geojson format does not support TIN geometry" ) );
  return {};
}
...
QgsTriangulatedSurface *QgsTriangulatedSurface::snappedToGrid( double hSpacing, double vSpacing, double dSpacing, double mSpacing, bool removeRedundantPoints ) const
{
  ...
  for ( QgsPolygon *patch : mPatches )
  {
    ...
    QgsCurve *exteriorRing = qgsgeometry_cast< QgsCurve *>( triangle->exteriorRing()->snappedToGrid( hSpacing, vSpacing, dSpacing, mSpacing, removeRedundantPoints ) );
    if ( !exteriorRing )
    {
      return nullptr;
    }

    QgsTriangle *gridifiedTriangle = new QgsTriangle;
    gridifiedTriangle->setExteriorRing( exteriorRing );
    surface->addPatch( gridifiedTriangle );
  }
