...
QgsPolyhedralSurface::QgsPolyhedralSurface( QgsMultiPolygon *multiPolygon )
{
  ...
  {
    mPatches.append( multiPolygon->polygonN( i ) );
  }
...
json QgsPolyhedralSurface::asJsonObject( int ) const
{
  QgsDebugError( QStringLiteral( "geojson format does not support PolyhedralSurface geometry" ) );
  return {};
}
...
QgsPolygon *QgsPolyhedralSurface::surfaceToPolygon() const
{
  QgsDebugMsgLevel( QStringLiteral( "QgsPolyhedralSurface does not handle surfaceToPolygon." ), 2 );
  return nullptr;
}
...
QgsPolyhedralSurface *QgsPolyhedralSurface::snappedToGrid( double hSpacing, double vSpacing, double dSpacing, double mSpacing, bool removeRedundantPoints ) const
{
  ...
  {
    // exterior ring
    QgsCurve *exteriorRing = static_cast< QgsCurve *>( patch->exteriorRing()->snappedToGrid( hSpacing, vSpacing, dSpacing, mSpacing, removeRedundantPoints ) );
    if ( !exteriorRing )
    {
      return nullptr;
    }

    QgsPolygon *gridifiedPatch = new QgsPolygon;
    gridifiedPatch->setExteriorRing( exteriorRing );
    ...
    for ( int i = 0, n = patch->numInteriorRings(); i < n; ++i )
    {
      ...
      QgsCurve *gridifiedInterior = static_cast< QgsCurve * >( interiorRing->snappedToGrid( hSpacing, vSpacing, dSpacing, mSpacing, removeRedundantPoints ) );
      if ( !gridifiedInterior )
        continue;

      gridifiedPatch->addInteriorRing( gridifiedInterior );
    }

    surface->addPatch( gridifiedPatch );
  }
...
QgsPolyhedralSurface *QgsPolyhedralSurface::simplifyByDistance( double tolerance ) const
{
  ...
  for ( QgsPolygon *polygon : mPatches )
  {
    simplifiedGeom->addPatch( polygon->simplifyByDistance( tolerance )->surfaceToPolygon() );
  }
