...
class CORE_EXPORT QgsSnappingConfig
{
  ...
  public:
     ...
     * \since QGIS 3.12
     */
    static const QString snappingTypeFlagToString( SnappingTypeFlag type, int maximumTypeVisible = 2 )
    {
      QString modes;
      int activeTypes = 0;

      if ( type.testFlag( QgsSnappingConfig::SnappingTypes::NoSnapFlag ) )
        return QObject::tr( "No Snapping" );

      std::map<QgsSnappingConfig::SnappingTypeFlag, QString> typesMap;
      typesMap.insert( { QgsSnappingConfig::SnappingTypes::VertexFlag, QObject::tr( "Vertex" ) } );
      typesMap.insert( { QgsSnappingConfig::SnappingTypes::SegmentFlag, QObject::tr( "Segment" ) } );
      typesMap.insert( { QgsSnappingConfig::SnappingTypes::AreaFlag, QObject::tr( "Area" ) } );
      typesMap.insert( { QgsSnappingConfig::SnappingTypes::CentroidFlag, QObject::tr( "Centroid" ) } );
      typesMap.insert( { QgsSnappingConfig::SnappingTypes::MiddleOfSegmentFlag, QObject::tr( "Middle of Segments" ) } );
      for ( auto it = typesMap.cbegin(); it != typesMap.cend(); ++it )
      {
        if ( type & it->first )
        {
          if ( activeTypes == maximumTypeVisible )
          {
            modes.append( QObject::tr( ", …" ) );
            break;
          }
          if ( activeTypes > 0 )
            modes.append( QObject::tr( ", " ) );
    ...
    class CORE_EXPORT IndividualLayerSettings
    {
      ...
      private:
        bool mValid = false;
        bool mEnabled = false;
        SnappingTypeFlag mType = SnappingTypes::VertexFlag;
        double mTolerance = 0;
