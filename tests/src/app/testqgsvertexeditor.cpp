...
void TestQgsVertexEditor::initTestCase()
{
  ...
  nurbsFeature.setGeometry( QgsGeometry( nurbs.release() ) );
  ...
  mLayerPolyBezier = std::make_unique<QgsVectorLayer>( u"NurbsCurve?crs=EPSG:27700"_s, u"layer poly-bezier"_s, u"memory"_s );
...
void TestQgsVertexEditor::testNurbsWeightColumn()
{
  ...
  QVERIFY( hdrs.contains( u"w"_s ) );

  QCOMPARE( mVertexEditor->mVertexModel->rowCount(), 4 );

  const int wCol = hdrs.indexOf( 'w'_L1 );
  QVERIFY( wCol >= 0 );
